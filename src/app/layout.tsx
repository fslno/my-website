...
export default function RootLayout({
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      ...
      <body className="font-body antialiased m-0 p-0 min-h-screen bg-[#F4F4F4] text-foreground overflow-x-hidden" suppressHydrationWarning>
        
        {/* BOOT OVERLAY: Branded Manifest to prevent visual flickering */}
        <div 
          className={cn(
            "fixed inset-0 z-[9999] bg-[#F4F4F4] flex items-center justify-center pointer-events-none transition-opacity duration-700",
  ...
  return (
    <>
      ...
      {(themeLoading || storeLoading) && !isAdmin && (
        <div className="fixed inset-0 bg-[#F4F4F4] z-[1000]" suppressHydrationWarning />
      )}
    </>
  );
}
