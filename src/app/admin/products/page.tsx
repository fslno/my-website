...
export default function ProductsPage() {
  ...
  const handleSaveProduct = async () => {
    ...
    const productData = {
      ...
      logistics: {
        weight: parseFloat(weight) || Number(shippingConfig?.defaultWeight) || 0,
        length: parseFloat(length) || Number(shippingConfig?.defaultLength) || 0,
        width: parseFloat(width) || Number(shippingConfig?.defaultWidth) || 0,
        height: parseFloat(height) || Number(shippingConfig?.defaultHeight) || 0,
        shippingClass
      },
      status: 'active',
      updatedAt: serverTimestamp(),
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                <TabsContent value="logistics" className="p-4 sm:p-8 m-0 space-y-12 max-w-5xl mx-auto">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-8"><div className="flex items-center gap-2 mb-2"><Scale className="h-4 w-4 text-gray-400" /><h3 className="text-[10px] uppercase tracking-widest font-bold text-gray-500">Shipping Info</h3></div><div className="grid gap-6"><div className="space-y-2"><Label className="text-[10px] uppercase font-bold text-gray-500">Weight (kg)</Label><Input type="number" step="0.01" placeholder={shippingConfig?.defaultWeight || "0.00"} value={weight} onChange={(e) => setWeight(e.target.value)} className="h-12 bg-white font-mono" /></div><div className="space-y-2"><Label className="text-[10px] uppercase font-bold text-gray-500">Shipping Type</Label><Select value={shippingClass} onValueChange={setShippingClass}><SelectTrigger className="h-12 bg-white"><SelectValue placeholder="Select type..." /></SelectTrigger><SelectContent><SelectItem value="standard" className="text-[10px] font-bold uppercase">Standard</SelectItem><SelectItem value="heavy" className="text-[10px] font-bold uppercase">Heavy</SelectItem><SelectItem value="fragile" className="text-[10px] font-bold uppercase">Fragile</SelectItem></SelectContent></Select></div></div></div>
                    <div className="space-y-8"><div className="flex items-center gap-2 mb-2"><Maximize2 className="h-4 w-4 text-gray-400" /><h3 className="text-[10px] uppercase tracking-widest font-bold text-gray-500">Box Dimensions</h3></div><div className="grid grid-cols-3 gap-4"><div className="space-y-2"><Label className="text-[8px] uppercase font-bold text-gray-400">L (cm)</Label><Input type="number" placeholder={shippingConfig?.defaultLength || "0"} value={length} onChange={(e) => setLength(e.target.value)} className="h-12 bg-white font-mono" /></div><div className="space-y-2"><Label className="text-[8px] uppercase font-bold text-gray-400">W (cm)</Label><Input type="number" placeholder={shippingConfig?.defaultWidth || "0"} value={width} onChange={(e) => setWidth(e.target.value)} className="h-12 bg-white font-mono" /></div><div className="space-y-2"><Label className="text-[8px] uppercase font-bold text-gray-400">H (cm)</Label><Input type="number" placeholder={shippingConfig?.defaultHeight || "0"} value={height} onChange={(e) => setHeight(e.target.value)} className="h-12 bg-white font-mono" /></div></div></div>
                  </div>
                </TabsContent>
              </div>
