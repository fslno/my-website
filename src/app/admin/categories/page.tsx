...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    const categoryData: any = {
      name, description, imageUrl: imageUrl || '', sizeChartId,
      seoTitle: seoTitle || name, seoDescription: seoDescription || description,
      updatedAt: new Date().toISOString()
    };
    if (editingId) {
      updateDoc(doc(db, 'categories', editingId), categoryData)
        .then(() => { setIsDialogOpen(false); resetForm(); toast({ title: "Updated", description: "Category saved." }); })
        .catch((error) => { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: `categories/${editingId}`, operation: 'update', requestResourceData: categoryData })); })
      ...
      addDoc(collection(db, 'categories'), newData)
        .then(() => { setIsDialogOpen(false); resetForm(); toast({ title: "Success", description: `${name} added.` }); })
        .catch((error) => { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'categories', operation: 'create', requestResourceData: newData })); })
        .finally(() => setIsSaving(false));
    }
  };
  ...
  const resetForm = () => { setName(''); setDescription(''); setImageUrl(''); setSizeChartId(''); setSeoTitle(''); setSeoDescription(''); setEditingId(null); setActiveTab('general'); };

  const openEdit = (category: any) => {
    setName(category.name || ''); setDescription(category.description || ''); setImageUrl(category.imageUrl || '');
    setSizeChartId(category.sizeChartId || ''); setSeoTitle(category.seoTitle || ''); setSeoDescription(category.seoDescription || '');
    setEditingId(category.id); setIsDialogOpen(true);
  };
    ...
    <div className="space-y-8 min-w-0">
      ...
      <div className="bg-white border rounded-none overflow-hidden shadow-sm">
        <div className="hidden lg:block">
          <Table>
            <TableHeader className="bg-gray-50/50"><TableRow className="border-black/5"><TableHead className="w-[100px] text-[10px] font-bold uppercase tracking-widest text-gray-500">Visual</TableHead><TableHead className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Name</TableHead><TableHead className="text-[10px] font-bold uppercase tracking-widest text-gray-500">SEO</TableHead><TableHead className="text-[10px] font-bold uppercase tracking-widest text-center">Order</TableHead><TableHead className="w-[100px]"></TableHead></TableRow></TableHeader>
            <TableBody>
              {categoriesLoading ? (
                ...
                <TableRow key={category.id} className="hover:bg-gray-50/50 cursor-pointer border-black/5 group" onClick={() => openEdit(category)}>
                  <TableCell><div className="w-14 h-14 bg-gray-100 relative overflow-hidden border shadow-sm">{category.imageUrl ? <Image src={category.imageUrl} alt={category.name} fill className="object-cover" /> : <ImageIcon className="h-6 w-6 text-gray-300 mx-auto" />}</div></TableCell>
                  <TableCell><div className="flex flex-col"><span className="font-bold text-sm uppercase">{category.name}</span><span className="text-[10px] text-gray-500 line-clamp-1 uppercase">{category.description}</span></div></TableCell>
                  <TableCell><Badge variant="outline" className={cn("text-[9px] font-bold uppercase border-none", category.seoTitle ? "bg-green-50 text-green-700" : "bg-orange-50 text-orange-700")}>{category.seoTitle ? 'OPTIMIZED' : 'PENDING'}</Badge></TableCell>
                  <TableCell><div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}><Button variant="ghost" size="icon" onClick={e => handleMoveOrder(category.id, 'up', e)} disabled={idx === 0}><ArrowUp className="h-3.5 w-3.5" /></Button><span className="text-[10px] font-mono font-bold w-6 text-center">{idx + 1}</span><Button variant="ghost" size="icon" onClick={e => handleMoveOrder(category.id, 'down', e)} disabled={idx === (categories?.length || 0) - 1}><ArrowDown className="h-3.5 w-3.5" /></Button></div></TableCell>
                  <TableCell><div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity"><Button variant="ghost" size="icon" onClick={e => handleDelete(category.id, e)}><Trash2 className="h-4 w-4 text-red-500" /></Button></div></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          ...
          {categoriesLoading ? (
            ...
            <div key={category.id} onClick={() => openEdit(category)} className="p-4 bg-white space-y-4">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3"><div className="w-12 h-16 bg-gray-100 relative border overflow-hidden shrink-0">{category.imageUrl ? <Image src={category.imageUrl} alt={category.name} fill className="object-cover" /> : <ImageIcon className="h-5 w-5 text-gray-200 mx-auto" />}</div><div className="space-y-0.5"><p className="font-bold text-sm uppercase tracking-tight">{category.name}</p><p className="text-[10px] text-gray-500 uppercase line-clamp-1">{category.description}</p></div></div>
                <Badge variant="outline" className={cn("text-[8px] font-bold uppercase border-none h-4 px-1.5", category.seoTitle ? "bg-green-50 text-green-700" : "bg-orange-50 text-orange-700")}>{category.seoTitle ? 'OK' : '...'}</Badge>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-black/5">
                <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}><Button variant="ghost" size="icon" className="h-8 w-8 border" onClick={e => handleMoveOrder(category.id, 'up', e)} disabled={idx === 0}><ArrowUp className="h-3 w-3" /></Button><span className="text-[10px] font-mono font-bold w-4 text-center">{idx + 1}</span><Button variant="ghost" size="icon" className="h-8 w-8 border" onClick={e => handleMoveOrder(category.id, 'down', e)} disabled={idx === (categories?.length || 0) - 1}><ArrowDown className="h-3 w-3" /></Button></div>
                <Button variant="ghost" size="icon" className="h-9 w-9 text-red-500" onClick={e => handleDelete(category.id, e)}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
        </div>
