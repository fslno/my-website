...
export function ApplePayPayment({ amount, orderData, onSuccess, validate }: ApplePayPaymentProps) {
    ...
    if (stripe && amount > 0) {
      ...
      pr.on('paymentmethod', async (ev) => {
        ...
        try {
          ...
          const tempRefId = `APPLE_PAY_PENDING_${Date.now()}_${Math.random().toString(36).substring(2, 9).toUpperCase()}`;
    ...
    return (
      <div className="p-4 bg-zinc-50 border border-dashed border-zinc-200 text-center" style={{ borderRadius: 'var(--btn-radius)' }}>
        <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest leading-none">
          Apple Pay is only available on Safari/iOS.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <p className="text-[9px] text-center mt-2 text-gray-400 uppercase font-bold tracking-widest leading-none">
        Native biometric checkout enabled
      </p>
    </div>
  );
}
