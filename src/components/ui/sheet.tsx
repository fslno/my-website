...
>(({ side = "right", className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-10 z-50 rounded-full bg-red-600 text-white p-2 shadow-xl ring-offset-background transition-all hover:bg-red-700 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-red-700">
          <X className="h-5 w-5" />
